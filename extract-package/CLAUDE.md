...
- **main.rs** — Entry point. Creates the 800x600 eframe window and launches the app.
- **app.rs** — UI and state management. Renders the egui interface and drives two background operations (preview loading, extraction) via `std::sync::mpsc` channels polled each frame with `try_recv()`.
...
**Important:** The `pathname` file has two lines. Only the first line is the asset path. Always use `.lines().next()` when reading it — using `trim()` alone leaves the second-line metadata embedded in the path, which breaks on Windows.
